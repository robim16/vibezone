        ...
        <div className='flex flex-col gap-9'>
            <div className="flex justify-between items-start">
                <div className='flex gap-5 items-start'>
                    <Image src={userData.profilePhoto} alt='profile photo' width={100} height={100} className='rounded-full md:max-lg:hidden' />
                    <div className='flex flex-col gap-3'>
                        <p className='text-light-1 text-heading3-bold max-sm:text-heading4-bold'>
                            {userData.firstName} {userData.lastName}
                        </p>
                        <p className='text-light-3 text-subtle-semibold max-sm:text-heading4-bold'>
                            {userData.username}
                        </p>
                        <div className="flex gap-7 text-small-bold max-sm:gap-4">
                            <div className='flex max-sm:flex-col gap-2 ietms-center max-sm:gap-0.5'>
                ...
                {user.id !== userData.clerkId && (
                    (isFollowing ? (
                        <PersonRemove sx={{ color: "#7857FF", cursor: "pointer", fontSize: "40px" }} 
                            onClick={() => handleFollow()} />
                    ) : (
                        <PersonAddAlt sx={{ color: "#7857FF", cursor: "pointer", fontSize: "40px" }}
                            onClick={() => handleFollow()} />
                    ))
                )}
            </div>
            ...
            <div className="flex gap-6">
                {tabs.map((tab) => {
                    <Link className={`tab ${activeTab === tab.name ? "bg-purple-1" : "bg-dark-2"}`}
